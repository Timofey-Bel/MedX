...
@section('page-title', 'Заказы организации')
...
<div class="section">
    <div class="tabs-header">
        <div class="tabs-list" id="tabsList">
            ...
            <a href="{{ route('lk.orders') }}" class="tab-link active">Заказы</a>
...
@forelse($orders as $order)
    <a href="{{ route('lk.orders.show', $order['id']) }}" style="text-decoration: none; color: inherit; display: block;">
        <div class="card" style="margin-bottom: 16px; cursor: pointer; transition: transform 0.2s ease, box-shadow 0.2s ease;">
            <div class="card-header">
                <div class="card-header-row">
                    <div>
                        <h3 style="font-size: 18px; margin-bottom: 4px;">Заказ #{{ $order['order_code'] ?? $order['id'] }}</h3>
                        <p style="font-size: 14px; color: var(--muted-fg);">{{ date('d.m.Y H:i', strtotime($order['created_at'])) }}</p>
                    </div>
            ...
            <div class="card-body">
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-top: 12px;">
                    <div>
                        ...
                        <p style="font-size: 14px; font-weight: 500;">{{ $order['name'] }}</p>
                        <p style="font-size: 13px; color: var(--muted-fg);">{{ $order['phone'] }}</p>
                    </div>
                    <div>
                        <label style="font-size: 13px; color: var(--muted-fg); display: block; margin-bottom: 4px;">Товаров</label>
                        <p style="font-size: 14px; font-weight: 500;">{{ $order['items_count'] }} шт.</p>
                    </div>
                    <div>
                        <label style="font-size: 13px; color: var(--muted-fg); display: block; margin-bottom: 4px;">Сумма</label>
                        <p style="font-size: 18px; font-weight: 700;">{{ number_format($order['total_amount'], 0, ',', ' ') }} ₽</p>
                    </div>
                </div>
            </div>
        </div>
