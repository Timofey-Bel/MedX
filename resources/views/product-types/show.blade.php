{{-- 
    Страница типа товара с его товарами
    Миграция из legacy: site/modules/sfera/product_type/product_type.tpl
    Дата создания: 2026-02-27
--}}
@extends('layouts.app')

@section('title', 'Товары типа: ' . $productType['name'] . ' - Творческий Центр СФЕРА')
...
<div class="breadcrumbs">
    <div class="container">
        ...
        <span>/</span>
        <a href="/product_types/">Типы товаров</a>
        <span>/</span>
...
<main class="catalog-page">
    <div class="container">
        <div class="catalog-layout catalog-layout-fullwidth">
            <!-- Main Content (без фильтров) -->
            <div class="catalog-content">
                <!-- Toolbar -->
                <div class="catalog-toolbar">
                    <div class="toolbar-left">
                        <h1>Товары типа: {{ $productType['name'] }}</h1>
                        <span class="results-count" id="resultsCount">Найдено {{ $total }} {{ $total == 1 ? 'книга' : 'книг' }}</span>
                    </div>
...
@if(count($products) > 0)
    @foreach($products as $product)
                    <article class="product-card" data-product-id="{{ $product['id'] }}" data-favorite-product-id="{{ $product['id'] }}">
                            ...
                            <div class="product-info">
                                ...
                                <div class="vi_24 vi0_24 p6b3_0_4-a p6b3_0_4-a0 p6b3_0_4-a1 tsBodyMBold"
                                    ...
                                    <span class="p6b3_0_4-a4">
                                        ...
                                        <span style="color: var(--textSecondary);">{{ number_format($product['reviews_count'], 0, '.', ' ') }}&nbsp;отзыв{{ $product['reviews_count'] > 1 ? 'ов' : '' }}</span>
                                    </span>
                                        @endif
                                    @endif
                                </div>
...
@if($pages > 1)
    @php
        $pagesInGroup = $endPage - $startPage + 1;
    @endphp
                <div class="pagination">
    @if($page > 1)
                    <a href="/product_type/?product_type_id={{ $productType['id'] }}&page={{ $page - 1 }}" class="pagination-btn">
    ...
    @for($pageNum = $startPage; $pageNum <= $endPage; $pageNum++)
        ...
        @else
                    <a href="/product_type/?product_type_id={{ $productType['id'] }}&page={{ $pageNum }}" class="pagination-btn">{{ $pageNum }}</a>
        @endif
    @endfor
    @if($page < $pages)
                    <a href="/product_type/?product_type_id={{ $productType['id'] }}&page={{ $page + 1 }}" class="pagination-btn">
