...
function CartViewModel() {
    ...
    // Алиасы для совместимости с checkout шаблоном
    self.promocode_input = ko.observable('');
    self.promocode_message = self.promoMessage; // используем существующий observable
    self.promocode_applied = self.promoCodeApplied; // используем существующий observable
    ...
    // Вычисляемое - массив выбранных товаров (для checkout)
    ...
    self.loadCart = function(serverData) {
        ...
        if (data) {
            ...
            if (data && data.promocode) {
                ...
                self.promocode_input(data.promocode); // синхронизируем с checkout полем
            }
    ...
    self.removeItem = function(item) {
        ...
        fetch('/api/cart?task=delete_product', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
    ...
    self.updateAmount = function(item) {
        ...
        fetch('/api/cart?task=update_amount', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
    ...
    self.clearCart = function() {
        ...
        fetch('/api/cart?task=clear_cart', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            }
    ...
    self.applyPromoCode = function() {
        console.log('=== Applying promo code ===', self.promocode_input());
        
        var promoCode = self.promocode_input().trim();
        if (!promoCode) {
        ...
        // Синхронизируем с основным полем промокода
        self.promoCode(promoCode);
        
        // Отправляем промокод на сервер
        fetch('/api/cart?task=save_promocode', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
        ...
        .then(function(data) {
            ...
            return fetch('/api/cart?task=get_cart', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                },
    ...
    self.removePromoCode = function() {
        ...
        self.promoCode('');
        self.promocode_input(''); // очищаем поле ввода
        
        // Отправляем запрос на удаление промокода
        fetch('/api/cart?task=del_promocode', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
