...
function setupSearch() {
    ...
    function fetchSearchSuggestions(query) {
        ...
        // Формируем URL для AJAX запроса (новый Laravel endpoint)
        const url = '/api/search/autocomplete?query=' + encodeURIComponent(query);
        
        fetch(url)
            .then(response => response.json())
            .then(data => {
                // Новый формат ответа: { suggestions: [...] }
    ...
    function displaySuggestions(products) {
        ...
        products.forEach(product => {
            // Простые текстовые ссылки без изображений
            html += `<a href="${product.url}" class="search-suggestion-item">
                <div class="suggestion-name">${product.name}</div>
                ${product.price > 0 ? `<span class="suggestion-price">${product.price} ₽</span>` : ''}
            </a>`;
        });
...
function setupCatalog() {
    ...
    function openCatalog() {
        ...
        if (categoryItems.length > 0) {
            ...
            // Show first category's subcategories
            ...
            const targetGroup = document.querySelector(`.catalog-subcategory-group[data-subcategory="${categoryId}"]`);
            if (targetGroup) {
